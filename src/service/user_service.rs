...
pub async fn get_users(app_state: web::Data<AppState<'_>>) -> Result<Vec<User>, Error> {
    app_state.context.user.get_users().await
}

pub async fn get_user(user_id: i32, app_state: web::Data<AppState<'_>>) -> Result<User, Error> {
    app_state.context.user.get_user(user_id).await
}
...
pub async fn create_user(
    ...
    app_state.context.user.create_user(&body).await
}
...
pub async fn update_user(
    ...
    app_state.context.user.update_user(user_id, &body).await
}
